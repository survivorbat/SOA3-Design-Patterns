...
trait EntityGetSetTestTrait
{
    ...
    /**
     * An empty ID should return an empty string or null if the method exists
     * @throws \ReflectionException
     */
    public function testGetIdIsEmptyString()
    {
        $this->validateConfiguration();
        $reflectionInstance = new \ReflectionClass($this->class);
        $this->validateTestedClass($reflectionInstance);

        if (!$reflectionInstance->hasMethod('getId')) {
            return;
        }

        $instance = new $this->class();
        $this->assertTrue(in_array($instance->getId(), [null, '']));
    }
